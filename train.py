...
def main(argv):
    ...
    # model = ResNet_perso_50(resnet_trainable_layers=0)
    ...
    #conf_mat = tfa.metrics.MultiLabelConfusionMatrix(num_classes=4)
    ...
    def train_step(images, labels):
        with tf.GradientTape() as tape:
            logits = model(images)
            loss = loss_func(tf.one_hot(labels, 2), logits)
    ...
    def test_step(images, labels):
        ...
        #conf_mat.update_state(labels, predictions)
    ...
    #print('Confusion matrix:', conf_mat.result().numpy())
    
    model.save_weights(os.path.join(
            './saved_model', '{}_{:06d}_bs{}.tf'.format('resnet_hub', step, FLAGS.batch_size)), save_format='tf')
